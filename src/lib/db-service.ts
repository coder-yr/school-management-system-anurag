...
export class DatabaseService {
  private readonly staffRoles: AppRole[] = ["super_admin", "admin", "teacher"];
  ...
  async recordPayment(
    ...
    try {
      if (!["student", "admin", "teacher", "super_admin"].includes(userRole)) {
  ...
  async approveLeaveRequest(requestId: string, approvedBy: string, userRole: AppRole): Promise<LeaveRequest> {
    try {
      if (userRole !== "admin" && userRole !== "super_admin") throw new AuthorizationError("Only admins can approve leave requests");
